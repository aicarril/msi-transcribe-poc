...
- **Do**: Open the Demo UI URL (see Live URLs below) in Chrome.
...
- **Do**: Read the sample dictation script (Section 6 below) clearly into your microphone.
...
- **Show**: A brief loading indicator, then the chart fields on the right panel auto-populate with extracted data:
...
- **Explain**: "The full transcript was sent to Amazon Bedrock — specifically Claude — which extracted the structured fields into the chart template. It only fills in what was actually mentioned. Fields without evidence are left empty, not hallucinated."

### Step 6: Review and Edit the Chart
...
### Step 7: Show Stored Data (Optional Technical Deep-Dive)

- **Say**: "Under the hood, let me show you what's stored."
- **Do**: Open the AWS Console → DynamoDB → Table: `msi-transcribe-poc-sessions` → Items. Click on the session you just created.
- **Show**: The DynamoDB item with `sessionId`, `chart` (JSON), `transcriptText`, `createdAt`, `status`.
...
|---|---|
| **Demo UI** | *(CloudFront URL — to be deployed in subtask 5)* |
| **API Endpoint** | `https://{api-id}.execute-api.us-east-1.amazonaws.com/prod/extract-chart` |
...
> We use Amazon Transcribe's Custom Vocabulary feature with 29 medical spa-specific terms — product names like Botox and Dysport, anatomical terms like glabella and nasolabial, and procedure terms like microneedling and dermaplaning. This significantly improves accuracy over the base model. In the demo you can hear terms transcribed correctly that would otherwise be garbled.

**Q3: What happens if the AI fills in a field incorrectly?**
> The provider always reviews the auto-filled chart before saving. They can edit any field. The system is designed as an assistant, not a replacement — it drafts the chart, the provider approves it. We also preserve the raw transcript so the chart can be re-extracted if needed.

**Q4: How long does the chart extraction take?**
> Typically 2-5 seconds after the session ends. We use Claude 3 Haiku on Bedrock, which is optimized for speed. For this use case, even 10 seconds is acceptable since the provider is transitioning between patients.

**Q5: Can this work with different chart templates?**
> Yes. The chart template is a JSON configuration. The POC uses the Neuromodulator Treatment Form, but the same architecture supports Filler, Microneedling, CoolSculpting, and all other MSI templates. The LLM prompt includes the template, so switching templates is just a configuration change.
...
> The POC exposes a REST API. Meevo would embed the transcription UI component and call the same API endpoints. The Cognito-based auth would be replaced with Meevo's existing authentication. The integration is straightforward — it's standard HTTPS API calls and WebSocket for audio streaming.
...
- Verify Cognito Identity Pool ID is correct in the page config
- Ensure you're in a region where Transcribe Streaming is available (us-east-1)

### Transcription is inaccurate for medical terms
- Confirm the Custom Vocabulary `msi-transcribe-poc-medical-spa` is in READY state
- Speak clearly and at a moderate pace
- Ensure the vocabulary name is correctly referenced in the Transcribe streaming call

### Chart extraction returns empty fields
- Check that the transcript contains enough detail (use the sample dictation)
- Verify the Lambda function has Bedrock permissions (check CloudWatch logs)
- Check API Gateway endpoint is responding (try a curl POST with sample text)

### API returns CORS error
- Ensure the OPTIONS method is deployed on the API Gateway resource
- Check that the Lambda returns `Access-Control-Allow-Origin: *` headers
- Redeploy the API Gateway stage if needed

### DynamoDB or S3 errors
- Check Lambda execution role has correct permissions
- Verify table name `msi-transcribe-poc-sessions` and bucket name match environment variables
- Check CloudWatch Logs for the Lambda function: `/aws/lambda/msi-transcribe-poc-extract-chart`

### Quick recovery: restart the demo
If something breaks mid-demo, say: "Let me show you the chart extraction piece directly." Open a terminal and run:
```bash
curl -X POST https://{api-id}.execute-api.us-east-1.amazonaws.com/prod/extract-chart \
  -H "Content-Type: application/json" \
  -d '{"transcriptText": "Patient received 20 units of Botox in the glabella and 12 units in the forehead. Informed consent obtained. No adverse reactions. Follow up in two weeks."}'
```
This bypasses the mic/transcription and shows the AI chart extraction directly.
